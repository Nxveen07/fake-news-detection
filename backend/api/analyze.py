...
from services.chatgpt_factcheck import analyze_with_chatgpt
...
def analyze_content(req: ContentRequest):
    ...
    def _chatgpt_to_result(chatgpt_out: dict) -> dict:
        ...
        source = "ChatGPT fact-checking API"
        if verify_steps:
        ...
        return {
            ...
            "fact_check_provider": "chatgpt",
            "cross_verification": cross_obj,
        }

    chatgpt_result = analyze_with_chatgpt(
    ...
    if chatgpt_result is not None:
        result = _chatgpt_to_result(chatgpt_result)
    else:
        raise HTTPException(
            status_code=503,
            detail="ChatGPT analysis failed or is unavailable. Please check your API key and connection.",
        )
