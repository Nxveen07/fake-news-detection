...
def analyze_with_chatgpt(headline: str, article_text: str, source_name: str | None = None) -> dict[str, Any] | None:
    """
    Returns structured fact-check JSON from ChatGPT (OpenAI API) or None when unavailable.
    """
    api_key = os.getenv("OPENAI_API_KEY", "").strip()
    if not api_key:
        return None

    model = os.getenv("OPENAI_MODEL", "gpt-3.5-turbo") # can also use gpt-4o, gpt-4o-mini
    ...
    if isinstance(cross_raw, dict):
        ...
        cross_verification = {
            ...
            "provider": "chatgpt_model_estimate",
        }
